...
describe('delegation-policy OMX parity surface', () => {
  ...
  afterEach(() => {
    if (saved.OMC_TEAM_CHILD_MODEL === undefined) delete process.env.OMC_TEAM_CHILD_MODEL;
    else process.env.OMC_TEAM_CHILD_MODEL = saved.OMC_TEAM_CHILD_MODEL;
    if (saved.OMX_TEAM_CHILD_MODEL === undefined) delete process.env.OMX_TEAM_CHILD_MODEL;
    else process.env.OMX_TEAM_CHILD_MODEL = saved.OMX_TEAM_CHILD_MODEL;
  });
